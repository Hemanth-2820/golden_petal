...
const Intro = () => {
  return (
    <section style={{ backgroundColor: '#F8F6F0', padding: '6rem 2rem', overflow: 'hidden' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        
        {/* Top Text Section */}
        <div style={{ marginBottom: '6rem' }}>
          <h2 className="title-display" style={{ fontSize: 'clamp(4rem, 10vw, 8rem)', lineHeight: 0.9, color: '#000', margin: 0, letterSpacing: '-2px' }}>
            GOLDEN PETAL,
          </h2>
          <div style={{ display: 'inline-block', position: 'relative', marginTop: '10px' }}>
            {/* Yellow Background Block */}
            <div style={{ position: 'absolute', top: 0, left: '-20px', right: '-20px', bottom: 0, backgroundColor: '#D7F16C', zIndex: 0, transform: 'rotate(-2deg)' }}></div>
            <h2 className="title-display" style={{ fontSize: 'clamp(4rem, 10vw, 8rem)', lineHeight: 0.9, color: '#000', margin: 0, position: 'relative', zIndex: 1, letterSpacing: '-2px', padding: '0 10px' }}>
              DECODED.
            </h2>
          </div>
          
          <div style={{ marginTop: '4rem', maxWidth: '800px' }}>
            <p style={{ fontSize: '1.6rem', fontWeight: 500, lineHeight: 1.6, color: '#111', marginBottom: '1.5rem' }}>
              Golden Petal is a celebration space where people can connect, celebrate, and create lasting memories.
            </p>
            <p style={{ fontSize: '1.6rem', fontWeight: 500, lineHeight: 1.6, color: '#111', marginBottom: '2rem' }}>
