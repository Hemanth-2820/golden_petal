...
const Addons = () => {
  const addons = [
    ...
    { name: 'WELCOME BOARD', price: 'Rs. 700/-', desc: 'An elegant, customized welcome board to greet your special guests.', img: '/new_addon_welcome.jpg' },
    ...
    { name: 'LED NUMBER', price: 'Rs. 150/- (Per Number)', desc: 'Light up your celebration with glowing LED numbers or custom messages.', img: '/addon_led.jpg' },
  ...
  return (
    <section id="addons" style={{ background: 'var(--bg-cream)', color: 'var(--text-dark-green)', padding: '4rem 2rem' }}>
      <div style={{ maxWidth: '1300px', margin: '0 auto', paddingBottom: '50vh' }}>
        ...
        {addons.map((addon, index) => (
          <div key={index} style={{ 
            ...
            background: 'var(--bg-cream)', // Solid background to cover the element underneath
            display: 'flex', 
            flexWrap: 'wrap',
            borderTop: '1px solid rgba(39, 57, 31, 0.3)', 
            padding: '4rem 0',
            ...
            <div style={{ flex: '1 1 300px', paddingRight: '2rem' }}>
              <h2 className="title-display" style={{ fontSize: 'clamp(2rem, 5vw, 4rem)', lineHeight: 1, margin: 0 }}>
                {addon.name}
              </h2>
            </div>
            ...
            <div style={{ flex: '1 1 600px', display: 'flex', gap: '2rem', flexWrap: 'wrap', alignItems: 'flex-start' }}>
              <img 
                src={addon.img} 
                alt={addon.name} 
                style={{ width: '400px', height: '250px', objectFit: 'cover', borderRadius: '15px', maxWidth: '100%', boxShadow: '0 10px 30px rgba(0,0,0,0.1)' }}
              />
              <div style={{ flex: 1, minWidth: '250px' }}>
                <p style={{ fontSize: '1.2rem', lineHeight: '1.6', marginBottom: '1rem' }}>
                  {addon.desc}
                </p>
                <div className="title-display" style={{ fontSize: '1.5rem', color: 'var(--text-black-green)' }}>
                  {addon.price}
                </div>
              </div>
            </div>
